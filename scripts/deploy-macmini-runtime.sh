...
ssh "$SSH_TARGET" "cd '$REMOTE_DIR' && PORT=$SMOKE_PORT .build/kbo-live-backend-macos/run-backend.command >/tmp/kbo-live-backend-$SMOKE_PORT.log 2>&1 & pid=\$!; for i in {1..20}; do if ! kill -0 \$pid 2>/dev/null; then cat /tmp/kbo-live-backend-$SMOKE_PORT.log; exit 1; fi; if curl -fsS --max-time 1 http://127.0.0.1:$SMOKE_PORT/health; then kill \$pid; wait \$pid 2>/dev/null || true; exit 0; fi; sleep 0.25; done; cat /tmp/kbo-live-backend-$SMOKE_PORT.log; kill \$pid 2>/dev/null || true; exit 1"
